...
class AuthService:
    ...
    def refresh_access_token(self, refresh_token: str):
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Access token refresh failed: {str(e)}"
            )
    ...
    def logout_user(self, access_token: str) -> None:
        try:
            ...
            if self.user_repository.is_token_blacklisted(access_token):
                raise HTTPException(
                    status_code=status.HTTP_401_UNAUTHORIZED,
            ...
            self.user_repository.add_blacklisted_token(access_token, expires_at)
